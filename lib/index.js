...
function refreshContent() {
  return term.setFiles(fileManager.files.map(file => file.getDisplayName()));
}

eventEmitter.on('triggerItem', item => {
  const file = fileManager.files.find(el => item.endsWith(el.name));
  if (file) {
    file.trigger();
  }
  refreshContent();
...
eventEmitter.on('applyModifications', async () => {
  ...
  term.updateFooter(modified.map(item => item.name));
  fileManager = FileManager.fromGit();
  refreshContent();
});

eventEmitter.on('getDiff', item => {
  const file = fileManager.files.find(el => item.endsWith(el.name));
  if (file) {
    term.diff.setContent(file.diff);
    term.diff.show();
  }
  refreshContent();
});

eventEmitter.on('refresh', refreshContent);
