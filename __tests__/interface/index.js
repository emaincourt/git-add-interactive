...
describe('#render', () => {
  it('renders all the components of the screen', () => {
    ...
    const term = new Interface();
    const mocks = {
      'screen.render': jest.spyOn(term.screen, 'render')
...
describe('#linkComponents', () => {
  it('links all the components of the screen', () => {
    ...
    const term = new Interface();
    const mocks = {
      'layout.append': jest.spyOn(term.layout, 'append')
        .mockImplementation(() => {}),
      'screen.append': jest.spyOn(term.screen, 'append')
        .mockImplementation(() => {}),
      'screen.render': jest.spyOn(term.screen, 'render')
        .mockImplementationOnce(() => {}),
      'list.focus': jest.spyOn(term.list, 'focus')
        .mockImplementationOnce(() => {}),
    };
    term.linkComponents();
    expect(mocks['layout.append'].mock.calls.length).toBe(6);
    expect(mocks['screen.append'].mock.calls.length).toBe(2);
...
describe('#setFiles', () => {
  it('sets the files of the list', () => {
    ...
    const term = new Interface();
    const mocks = {
      'list.setItems': jest.spyOn(term.list, 'setItems')
        .mockImplementationOnce(() => {}),
    };
    term.setFiles([]);
...
describe('#updateFooter', () => {
  it('sets the files of the list', () => {
    ...
    const term = new Interface();
    const mocks = {
      'footer.setContent': jest.spyOn(term.footer, 'setContent')
        .mockImplementationOnce(() => {}),
    };
    term.updateFooter([]);
